...
export default function Modal({ }) {
    const { premiumThankYouModal, togglePremiumThankYouModal} = useAllContext();

    console.log(premiumThankYouModal)

  // prevent background scroll when modal is open
  useEffect(() => {
    if (premiumThankYouModal) {
  ...
  if (!premiumThankYouModal) return null; // don’t render if closed

  return (
    <div className={`fixed inset-0 z-50 items-center justify-center ${premiumThankYouModal ? 'flex' : 'hidden' }`}>
      {/* backdrop */}
      <div
        className="absolute inset-0 bg-black/50"
        onClick={togglePremiumThankYouModal}
      ></div>
      ...
      <div className="border border-[8px] border-b-[#F97502] border-x-0 border-t-0 relative bg-white rounded-lg shadow-lg z-10 w-[375px] md:w-[624px] min-h-[473px] rounded-[16px] p-[40px] overflow-y-auto mt-[40px]">
        <div className="flex justify-between items-center">
            ...
            <Image src="/icons/close-square.png" width={24} height={24} alt="close thribe modal" className="cursor-pointer" onClick={togglePremiumThankYouModal} />
        </div>
        ...
        <Link href="/#joinCommunity" onClick={togglePremiumThankYouModal}>
